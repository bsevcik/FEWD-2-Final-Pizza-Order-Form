var runningTotalVar = 0,
  selectedDough,
  previousDoughAndSize,
  previousDoughPickIndex = false;
var pizzaSizes = {
  handTossed: {
    "Small ($9.99)": 9.99,
    "Medium ($12.99)": 12.99,
...
function whichDoughPicked(e) {
  ...
  for (var i = 0; i < allOptions.length; i++) {
    ...
    if (allOptions[i].checked == true) {
      ...
      document
        .getElementById("finishBuildingPizzaContainer")
        .classList.remove("hidden");
      document
        .getElementById("size")
        .parentElement.parentElement.classList.remove("hidden");
      document
        .getElementById("cheeseOptionsContainer")
        .classList.remove("hidden");
      document
        .getElementById("sauceOptionsContainer")
        .classList.remove("hidden");
      document
        .getElementById("additionalToppingsContainer")
        .classList.remove("hidden");

      //The seeThrough class is supposed to allow for css transitions. Changing from display: hidden; doesn't do transitions
      document
        .getElementById("size")
        .parentElement.parentElement.classList.remove("seeThrough");
      document
        .getElementById("cheeseOptionsContainer")
        .classList.remove("seeThrough");
      document
        .getElementById("sauceOptionsContainer")
        .classList.remove("seeThrough");
      document
...
window.addEventListener("load", function() {
  ...
  document.getElementById("addressType").addEventListener("change", function() {
    ...
    if (document.getElementById("addressType")[6].selected) {
      document
        .getElementById("otherAddressTypeContainer")
        .classList.remove("hidden");
    } else {
      document
        .getElementById("otherAddressTypeContainer")
  ...
  document
    .getElementById("continueToPizzaSelection")
    .addEventListener("click", function() {
      //The error message will have a list of issues. If there's no list items it'll get hidden
      var errorMessage =
        "<h1>Error!</h1><p>Please fix the following issues so we can deliver your pizza!</p><ul>";
      ...
      if (
        document.getElementById("name").value.match(/[^A-Z^a-z\s]/) !== null
      ) {
        errorMessage +=
          "<li>Name field has an invalid character. Only letters are allowed</li>";
        document.getElementById("name").classList.add("errorInput");
      ...
      if (
        document.getElementById("otherAddressType").value === "" &&
        document.getElementById("addressType").value === "Other"
      ) {
        errorMessage += "<li>Other Address Type must be filled in </li>";
        document.getElementById("otherAddressType").classList.add("errorInput");
      } else {
        document
          .getElementById("otherAddressType")
      ...
      if (document.getElementById("state").value === "") {
        ...
        document.getElementById("state").value.match(/[^A-Z^a-z]/) !== null &&
        document.getElementById("state").value.length !== 2
      ) {
        errorMessage +=
          "<li>State Field Has An Invalid Non-Letter Character</li><li>State Field Must Have Exactly Two Letters</li>";
        document.getElementById("state").classList.add("errorInput");
      } else if (
        document.getElementById("state").value.match(/[^A-Z^a-z]/) !== null
      ) {
        errorMessage +=
          "<li>State Field Has An Invalid Non-Letter Character</li>";
        document.getElementById("state").classList.add("errorInput");
      ...
      if (document.getElementById("zip").value === "") {
        ...
        document.getElementById("zip").value.match(/[^0-9]/) !== null ||
  ...
  window.document
    .getElementById("doughRadioButtons")
    .addEventListener("click", function(e) {
      if (previousDoughPickIndex !== false && e.srcElement.id !== "") {
        var confirmChange = window.confirm(
          "Are you sure you want to change your Dough Selection?\rSize options and prices change depending on the dough so you may need to select the size again."
        );
        if (confirmChange == true) {
          whichDoughPicked(e);
        } else if (confirmChange == false) {
          document.getElementsByName("doughPick")[
  ...
  for (var i = 0; i < whatToCalculate.length; i++) {
    whatToCalculate[i].addEventListener("change", function(e) {
  ...
  document
    .getElementById("finishBuildingPizzaButton")
    .addEventListener("click", function() {
      if (
        window.confirm(
          "Confirm Your Order Before Moving On To Billing Information"
        )
      ) {
  ...
  document
    .getElementById("sameAsDeliveryInformation")
    .addEventListener("change", function() {
      if (document.getElementById("sameAsDeliveryInformation").checked) {
        document.getElementById("nameb").value = document.getElementById(
          "name"
        ).value;
        document.getElementById(
          "streetAddressb"
        ).value = document.getElementById("streetAddress").value;
        document.getElementById("cityb").value = document.getElementById(
          "city"
        ).value;
        document.getElementById("stateb").value = document.getElementById(
          "state"
        ).value;
        document.getElementById("zipb").value = document.getElementById(
          "zip"
        ).value;
      } else {
  ...
  document
    ...
    .addEventListener("keyup", function() {
      if (document.getElementById("creditCardNumber").value.indexOf(4) === 0) {
        ...
        document.getElementById("creditCardNumber").value.indexOf(37) === 0
      ) {
  ...
  document.getElementById("submitOrder").addEventListener("click", function() {
    ...
    if (
      document.getElementById("aptb").value !== "" &&
      document.getElementById("aptb").value.match(/[^A-Z^a-z^0-9]/) !== null
    ) {
      errorMessage +=
        "<li>Apt, STE, or Room Number May Only Have Letters or Numbers </li>";
    ...
    if (document.getElementById("stateb").value === "") {
      ...
      document.getElementById("stateb").value.match(/[^A-Z^a-z]/) !== null &&
      document.getElementById("state").value.length !== 2
    ) {
      errorMessage +=
        "<li>State Field Has An Invalid Non-Letter Character</li><li>State Field Must Have Exactly Two Letters</li>";
    ...
    if (
      document.getElementById("creditCardNumber").value.match(/[^0-9]/) !== null
    ) {
      ...
      document.getElementById("creditCardNumber").value.length !== 13 &&
      document.getElementById("creditCardNumber").value.length !== 15 &&
      document.getElementById("creditCardNumber").value.length !== 16
    ) {
      document.getElementById("cardError").innerHTML =
        "The Card Number Must Be 13, 15, or 16 Digits Long";
      document.getElementById("cardError").classList.add("errorInput");
      document.getElementById("creditCardNumber").classList.add("errorInput");
      errorMessage += "<li>Credit Card Length is Invalid</li>";
    } else if (
      luhnValidator(document.getElementById("creditCardNumber").value) === false
    ) {
      document.getElementById("cardError").innerHTML =
    ...
    if (document.getElementById("cvcb").value.match(/[^0-9]/) !== null) {
      ...
      document.getElementById("cvcb").value.length !== 3 &&
      document.getElementById("cvcb").value.length !== 4
    ) {
      document.getElementById("cvcb").classList.add("errorInput");
    ...
    if (document.getElementById("expirationMonthb").value === "0") {
      ...
      currentMonth.getMonth() + 1 >
        Number(document.getElementById("expirationMonthb").value) &&
      currentMonth.getFullYear() + 1 >=
        Number(document.getElementById("expirationYearb").value)
    ) {
      document.getElementById("expirationMonthb").classList.add("errorInput");
      document.getElementById("expirationYearb").classList.add("errorInput");
      ...
      document
        .getElementById("expirationMonthb")
        .classList.remove("errorInput");
    }
